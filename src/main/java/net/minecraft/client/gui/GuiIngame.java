...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (scoreobjective1 != null) {
            if (Simp.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
                this.renderSimpScoreboard(scoreobjective1, scaledresolution);
            } else {
    ...
    public void renderSimpScoreboard(ScoreObjective scoreObjective, ScaledResolution resolution) {
        ...
        // Limit the number of scores to the top 15
        ...
        // Calculate the maximum width required for rendering
        ...
        // Get font height
        int fontHeight = useMcFont ? this.mc.fontRendererObj.FONT_HEIGHT : customFr.FONT_HEIGHT;
        ...
        if (!Simp.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            startX = resolution.getScaledWidth() - maxWidth;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.Vanilla && Simp.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            startX = resolution.getScaledWidth() - maxWidth;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.Left && Simp.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            if (startX == 0 || resolution.getScaledWidth() == 0 || maxWidth == 0)
                return;
            startX = resolution.getScaledWidth() / maxWidth - 14;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.LeftOffset && Simp.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            if (startX == 0 || resolution.getScaledWidth() == 0 || maxWidth == 0)
                return;
            startX = resolution.getScaledWidth() / maxWidth;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.VanillaOffset && Simp.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            startX = resolution.getScaledWidth() - maxWidth - 14;
        }
        ...
        for (Score score : displayedScores) {
            ...
            String scorePoints = EnumChatFormatting.RED + "";

            if (score.getScorePoints() == 0) {
                playerName = EnumChatFormatting.YELLOW + "Simp Client";
            }
            ...
            if (useMcFont) {
                ...
                customFr.drawString(playerName, x + (maxWidth - playerNameWidth - scorePointsWidth) / 2, (float) yPos, textColor, false);
                customFr.drawString(scorePoints, x + maxWidth - scorePointsWidth, (float) yPos, textColor, false);
            }

            // Draw the header if it's the last score
            if (rowIndex == displayedScores.size()) {
                ...
                if (useMcFont) {
                    ...
                    customFr.drawString(header, x + (maxWidth - customFr.getStringWidth(header)) / 2, (float) (yPos - fontHeight), textColor, false);
                }
            }
        }
    }
