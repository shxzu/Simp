...
public class TrajectoriesModule extends Module {
    ...
    private final NumberProperty width = new NumberProperty("Width", 6, 0, 12, 1);
    ...
    public final Listener<Render3DEvent> render3DEventListener = e -> {
        ...
        if (itemStack != null && (itemStack.getItem() instanceof ItemSnowball || itemStack.getItem() instanceof ItemEgg || itemStack.getItem() instanceof ItemBow || itemStack.getItem() instanceof ItemEnderPearl)) {
            ...
            rotationYaw = (prevRotationYaw = (float)(MathHelper.atan2(x, z) * 180.0 / 3.141592653589793));
            float prevRotationPitch;
            rotationPitch = (prevRotationPitch = (float)(MathHelper.atan2(y, MathHelper.sqrt_double(x * x + z * z)) * 180.0 / 3.141592653589793));
            boolean b = true;
            int ticksInAir = 0;
            while (b) {
                if (ticksInAir > 300) {
                ...
                rotationYaw = (float)(MathHelper.atan2(motionX, motionZ) * 180.0 / 3.141592653589793);
                for (rotationPitch = (float)(MathHelper.atan2(motionY, f3) * 180.0 / 3.141592653589793); rotationPitch - prevRotationPitch < -180.0f; prevRotationPitch -= 360.0f) {}
            ...
            if (this.positions.size() > 1) {
                ...
                GL11.glColor4f(ColorProcess.getColor().getRed() / 255.0f, ColorProcess.getColor().getGreen() / 255.0f, ColorProcess.getColor().getBlue() / 255.0f, 0.7f);
                GL11.glLineWidth(width.getValue().floatValue() / 2.0f);
                final Tessellator tessellator = Tessellator.getInstance();
                final WorldRenderer worldrenderer = tessellator.getWorldRenderer();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                worldrenderer.begin(3, DefaultVertexFormats.POSITION);
                for (final Vec3 vec5 : this.positions) {
                    worldrenderer.pos((float)vec5.xCoord - mc.getRenderManager().getRenderPosX(), (float)vec5.yCoord - mc.getRenderManager().getRenderPosY(), (float)vec5.zCoord - mc.getRenderManager().getRenderPosZ()).endVertex();
                }
                tessellator.draw();
                if (m != null) {
                    GL11.glColor4f(ColorProcess.getColor().getRed() / 255.0f, ColorProcess.getColor().getGreen() / 255.0f, ColorProcess.getColor().getBlue() / 255.0f, 0.3f);
