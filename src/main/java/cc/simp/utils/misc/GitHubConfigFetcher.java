...
public class GitHubConfigFetcher {
    ...
    public static boolean downloadAndLoadConfig(String configName) {
        try {
            ...
            java.io.File configFile = new java.io.File(cc.simp.api.config.ConfigManager.CONFIGS_DIR, configName + ".json");
            Files.write(Paths.get(configFile.getPath()), content.toString().getBytes());

            // Load the config
            return cc.simp.Simp.INSTANCE.getConfigManager().loadConfig(configName);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
