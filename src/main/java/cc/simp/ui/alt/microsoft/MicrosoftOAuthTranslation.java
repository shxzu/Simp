...
public class MicrosoftOAuthTranslation {
	...
	public MicrosoftAuthResult getAccessToken() throws Exception {
		...
		httpServer.createContext("/", exchange -> {
			...
			/*
			 * String oauth = Http.postURL(token, Map.of( "client_id",
			 * "2fa2913a-ebf9-4bb7-b16a-c3e7e437b5bc", "code", code, "grant_type",
			 * "authorization_code", "redirect_uri", "http://localhost:8080/",
			...
			try {
				...
				String oauth = http.readPost("https://login.live.com/oauth20_token.srf", toMap(tokenMap),
						"application/x-www-form-urlencoded");

				// String access_token = Http.gson().fromJson(oauth,
				...
				JSONObject xbl = new JSONObject(http.readPost("https://user.auth.xboxlive.com/user/authenticate",
						authObj, "application/json", "application/json"));

				String xbl_token = xbl.getString("Token");
				String xbl_uhs = xbl.getJSONObject("DisplayClaims").getJSONArray("xui").getJSONObject(0)
						.getString("uhs");
				...
				JSONObject xsts = new JSONObject(http.readPost("https://xsts.auth.xboxlive.com/xsts/authorize",
						authObj2, "application/json", "application/json"));

				String xsts_token = xsts.getString("Token");
				String xsts_uhs = xsts.getJSONObject("DisplayClaims").getJSONArray("xui").getJSONObject(0)
						.getString("uhs");
				...
				JSONObject resp = new JSONObject(
						http.readPost("https://api.minecraftservices.com/authentication/login_with_xbox", xboxAuth,
								"application/json"));

				MinecraftProfile profile = http.getJson("https://api.minecraftservices.com/minecraft/profile",
						resp.getString("access_token"), MinecraftProfile.class);

				response = new MicrosoftAuthResult(profile, resp.getString("access_token"));
				Session session = new Session(response.getProfile().getName(), response.getProfile().getId(),
						response.getAccessToken(), "legacy");
				Minecraft.getMinecraft().session = session;

				String success = "Successfully logged into account "
						+ Minecraft.getMinecraft().session.getProfile().getName();
